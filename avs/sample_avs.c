...
void SAMPLE_AVS_Usage(char* sPrgNm)
{
    ...
    printf("\t 3) 2 fisheye stitching, Image stabilizing.\n");
    printf("\t 4) Generate lut.\n");
    printf("\t 5) Position query, DEST->SRC.\n");
    printf("\t 6) Position query, SRC->DEST.\n");
...
HI_S32 SAMPLE_AVS_2Fisheye_Equirectangular_Cylindrical_Rectilinear(void)
{
    ...
    HI_BOOL              abChnEn[VPSS_MAX_PHY_CHN_NUM] = {0, 1, 0, 0};
    ...
    *************************************************/
    ...
    stViConfig.astViInfo[1].stSnsInfo.s32BusId      = 3;
    ...
    *************************************************/
    ...
    astVpssChnAttr[1].enCompressMode               = COMPRESS_MODE_TILE;
    astVpssChnAttr[1].enDynamicRange               = DYNAMIC_RANGE_SDR8;
    astVpssChnAttr[1].enVideoFormat                = VIDEO_FORMAT_TILE_16x8;
    ...
    astVpssChnAttr[1].stAspectRatio.u32BgColor     = 0xff;
...
HI_S32 SAMPLE_AVS_4Fisheye_CubeMap(void)
{
    ...
    *************************************************/
    ...
    stViConfig.astViInfo[2].stDevInfo.ViDev         = 3;
    stViConfig.astViInfo[2].stPipeInfo.aPipe[0]     = 2;
    stViConfig.astViInfo[2].stSnsInfo.MipiDev       = 3;
    stViConfig.astViInfo[2].stSnsInfo.s32BusId      = 5;

    stViConfig.as32WorkingViId[3]                   = 3;
    stViConfig.astViInfo[3].stDevInfo.ViDev         = 4;
    stViConfig.astViInfo[3].stPipeInfo.aPipe[0]     = 3;
    stViConfig.astViInfo[3].stSnsInfo.MipiDev       = 4;
    stViConfig.astViInfo[3].stSnsInfo.s32BusId      = 6;
    ...
    *************************************************/
    ...
    for (i = 0; i < u32PipeNum; i++)
    {
        snprintf(stAVSConfig.pLUTName[i], 64, "./data/avsp_mesh_out_%d.bin", i+1);
    }
...
HI_S32 SAMPLE_AVS_4NoBlend(void)
{
    ...
    *************************************************/
    ...
    u32BlkSize = COMMON_GetPicBufferSize(stSize.u32Width * 2, stSize.u32Height * 2, PIXEL_FORMAT_YVU_SEMIPLANAR_420,
                                        DATA_BITWIDTH_8, COMPRESS_MODE_SEG, DEFAULT_ALIGN);
    stVbConf.astCommPool[1].u64BlkSize = u32BlkSize;
    stVbConf.astCommPool[1].u32BlkCnt  = 6;
...
HI_S32 SAMPLE_AVS_GenerateLUT(void)
{
    ...
    stLutMask.u64MaskVirAddr[0] = (HI_U64)(HI_U32)pMaskVirAddr;
    ...
    {
        u64LutOutput[i] = (HI_U64)(HI_U32)pLUTVirAddr + i * AVS_LUT_SIZE;
    }
    ...
    stLutInput.u64FileInputVirAddr           = (HI_U64)(HI_U32)g_CalFile;
    ...
    for (i = 0; i < camera_num; i++)
    {
        ...
        fwrite((HI_CHAR*)(HI_U32)u64LutOutput[i], AVS_LUT_SIZE, 1, LutFILE);
...
HI_S32 SAMPLE_AVS_Dest2Src(void)
{
    ...
    for (i = 0; i < stAvsConfig.u32Camera; i++)
    {
        ...
        snprintf(LutPath, 128, "./data/avsp_mesh_out_%d.bin", i+1);
    ...
    for (i = 0; i < stAvsConfig.u32Camera; i++)
    {
        ...
        if (1 != fwrite((HI_S16*)(HI_UL)u64MeshVirAddr[i], u32MeshSize, 1, fp))
        {
    ...
    stDstPointIn.s32X = 1000;
    stDstPointIn.s32Y = 1000;
...
#endif
{
    ...
    {
        case '0':
            if ((SONY_IMX290_SLAVE_MIPI_2M_60FPS_10BIT != SENSOR0_TYPE)
                || (SONY_IMX290_SLAVE_MIPI_2M_60FPS_10BIT != SENSOR1_TYPE))
            {
                SAMPLE_PRT("Please configure SENSOR0~1 SONY_IMX290_SLAVE_MIPI_2M_60FPS_10BIT to support this case!\n");
                break;
            }
        ...
        case '1':
            if ((SONY_IMX290_SLAVE_MIPI_2M_60FPS_10BIT != SENSOR0_TYPE)
                || (SONY_IMX290_SLAVE_MIPI_2M_60FPS_10BIT != SENSOR1_TYPE)
                || (SONY_IMX290_SLAVE_MIPI_2M_60FPS_10BIT != SENSOR2_TYPE)
                || (SONY_IMX290_SLAVE_MIPI_2M_60FPS_10BIT != SENSOR3_TYPE))
            {
                SAMPLE_PRT("Please configure SENSOR0~3 SONY_IMX290_SLAVE_MIPI_2M_60FPS_10BIT to support this case!\n");
                break;
            }
