...
static HI_S32 SAMPLE_FISHEYE_StartViVoVenc(SAMPLE_VI_CONFIG_S* pstViConfig,VI_PIPE ViPipe, VI_CHN ViExtChn, VENC_CHN VencChn, VO_CHN VoChn,SIZE_S *pstDstSize)
{
    ...
    ******************************************/
    ...
    s32Ret = SAMPLE_COMM_VENC_Start(VencChn, g_enVencType, \
                                        g_enPicSize, g_enRcMode,0,&stGopAttr);
    if (HI_SUCCESS != s32Ret)
    {
...
static HI_VOID *SAMPLE_FISHEYE_Nine_Lattice_Thread(HI_VOID *arg)
{
    ...
    while ( HI_TRUE == bSetFisheyeAttr )
    {
        ...
        u32BufSize = COMMON_GetPicBufferSize(u32Width, u32Height,PIXEL_FORMAT_YVU_SEMIPLANAR_420,DATA_BITWIDTH_8,COMPRESS_MODE_NONE, 0);
        ...
        stVgsTask.stImgOut.stVFrame.u64VirAddr[0]    = (HI_U32)pu8OutVirtAddr + (HI_U64)u32OutStride * u32OutHeight * 2 / 3 + ALIGN_DOWN(u32OutWidth *2/3,u32XAlign);
        stVgsTask.stImgOut.stVFrame.u64VirAddr[1]    = (HI_U32)pu8OutVirtAddr + (HI_U64)u32OutStride * u32OutHeight + ((HI_U64)u32OutStride * u32OutHeight * 2/3) / 2 +  ALIGN_DOWN(u32OutWidth*2/3,u32XAlign);
        stVgsTask.stImgOut.stVFrame.u64VirAddr[2]    = (HI_U32)pu8OutVirtAddr + (HI_U64)u32OutStride * u32OutHeight + ((HI_U64)u32OutStride * u32OutHeight * 2/3) / 2 +  ALIGN_DOWN(u32OutWidth*2/3,u32XAlign);
...
HI_S32 SAMPLE_FISHEYE_360Panorama_Desktop_and_2normal(SAMPLE_VI_CONFIG_S* pstViConfig, VI_PIPE ViPipe)
{
    ...
    /* comm video buffer */
    u64BlkSize = COMMON_GetPicBufferSize(stSize.u32Width,stSize.u32Height,SAMPLE_PIXEL_FORMAT,DATA_BITWIDTH_8,HI_FALSE,0);
    stVbConf.astCommPool[0].u64BlkSize = u64BlkSize;
